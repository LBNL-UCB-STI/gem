...
prep.inputs.personal.charging <- function(exper.row,common.inputs){
  ###########################################################
  ...
  #TOOL USER INPUT: Desired size of fleet. Integer.
  fleet_size <- 1000000
  ...
  load_file_name <- paste0(workingDir,"/data/evi_load_profile_data_2019-01-05.RData") #Source of data, or file name to save to if re-calc
  if(!exists("evi_load_profiles")) {
    ...
    if(run_load_calc_func) {
      ...
      save(evi_load_profiles, file=load_file_name)
      print(Sys.time())
  ...
  inputs$parameters$personalEVChargeEnergyUB <- all.energy.constraints[,list(t,value=max.c)]
  inputs$parameters$personalEVChargeEnergyLB <- all.energy.constraints[,list(t,value=min.c)]
