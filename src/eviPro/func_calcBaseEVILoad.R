...
calcBaseEVILoad <- function(activity_data,time_step) {
  ...
  evi_load_profiles <- activity_data[,.(unique_vid,pev_type,schedule_vmt,dest_type,dest_chg_level,start_time,end_time_chg,avg_kw,kwh)]
  setkey(evi_load_profiles,unique_vid,start_time)
  
  #Create a unique key to use for creating a time sequence for each charging event
  evi_load_profiles[,session_id:=1:.N]
  ...
  evi_load_profiles <- evi_load_profiles[,cbind(time_of_day=seq(from=(ceiling(start_time*24/time_step)*time_step),
